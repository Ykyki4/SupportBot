...
python load.py
```
Данный скрипт обучает вашего бота. Из файла указанного в ```QUESTIONS_FILE_PATH``` берётся название нового intent, далее сообщения для обучения, и ответ который будет отправлять бот, при получении подобного сообщения. Чтобы лучше понимать как это работает, прочитайте эту [документацию](https://dialogflow.com/).
